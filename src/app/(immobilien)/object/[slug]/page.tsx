...
export default function PropertyDetailPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(false);
    fetch(`http://localhost:3000/property/slug/${slug}`)
      .then(res => {
        if (!res.ok) throw new Error('Not found');
        return res.json();
      })
      .then(data => {
        const operationType = data.operationType || data.operation || data.operation_type || undefined;
        setProperty({ ...data, operationType });
      })
      .catch(() => setError(true))
      ...
      <header className="w-full flex flex-col items-start gap-1 pb-2 mt-4 mb-2 max-w-5xl mx-auto px-2">
        <div className="w-full flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            {(() => {
              const rawType = property.operationType;
          ...
          {(() => {
            ...
            return (
              <span className="text-sm font-semibold text-primary">
                {priceLabel}: {property.price ?? (property.price_amount ? `${property.price_amount} ${property.currency || ''}` : '—')}
              </span>
            );
          })()}
        </div>
        <div className="flex items-center gap-1 mt-1">
          ...
          <span className="text-xs text-card-text-l dark:text-card-text-d">{property.city}{property.postal_code ? `, ${property.postal_code}` : ''}</span>
        </div>
      </header>
      <section className="w-full flex flex-col items-center mb-8">
        <ObjectSlideshow images={property.images && property.images.length > 0 ? property.images : [
